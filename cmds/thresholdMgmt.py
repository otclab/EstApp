        ...
        if len(args) < 4:
            ...
            if len(args) < 5:
                ...
                try:
                    ...
                    print(
                        "Error : %s no es un número o esta fuera de rango." % args[4])
                    card.close()
                    sys.exit(1)
...
class ThresholdList(list):
    def __init__(self, filename=None, card=None):
        wb = load_workbook(filename=filename)
        ws = wb.active

        # Se validan los umbrales
        super().__init__()
        for n, row in enumerate(ws[2:card.tapLimit+1]):
            ...
            if sup is None:
                raise ValueError(
                    f'Falta definir el umbral superior del tap {n+1}.')

            if not isinstance(sup, (int, float)):
                raise ValueError(
                    f'El umbral superior del tap {n+1} no es un número.')

            if inf is None:
                raise ValueError(
                    f'Falta definir el umbral inferior del tap {n+1}.')

            if not isinstance(inf, (int, float)):
                raise ValueError(
                    f'El umbral inferior del tap { n+1} no es un número.')

            if sup <= inf:
                raise ValueError(
                    'Los umbrales del tap {n+1} estan invertidos.')
...
def read_xls(card, xls_name):
    try:
        threshold_list = ThresholdList(filename=xls_name, card=card)
    except ValueError as e:
...
def write_xls(card, xls_name):
    ...
    for n, th in enumerate(card.threshold[:card.threshold.len]):
        ...
        ws[f'A{n+2}'].border = Border(top=thin,
                                      left=thick, right=thick, bottom=bottom)
        ws[f'B{n+2}'].border = Border(top=thin,
                                      left=thick, right=thin, bottom=bottom)
        ws[f'C{n+2}'].border = Border(top=thin,
                                      left=thin, right=thick, bottom=bottom)
